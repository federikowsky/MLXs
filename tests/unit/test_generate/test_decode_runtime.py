...
def _enable_async_eval(monkeypatch: pytest.MonkeyPatch) -> list[tuple[Any, ...]]:
...
def test_prepare_decode_plan_resolves_tensor_step_and_sync_policy() -> None:
    ...
    assert async_plan.sync.token_boundary.reason == "global_async"
...
def test_prepare_decode_plan_selective_split_async_is_narrowly_gated(
    ...
    assert eligible.sync.token_boundary.reason == "long_regime"
...
def test_sync_policy_records_common_token_boundary_diagnostics(
    ...
    assert profile["token_boundary_reason"] == "global_async"
...
def test_generate_async_eval_preserves_sequence_parity_and_uses_async_eval(
    ...
    calls = _enable_async_eval(monkeypatch)
    async_events = list(
        generate(
            _TableModel(transitions),
            cast(TokenizerProtocol, tokenizer),
    ...
    assert _event_summary(async_events) == _event_summary(baseline_events)
    assert calls


def test_generate_async_eval_preserves_stop_sequence_behavior(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls = _enable_async_eval(monkeypatch)
...
def test_generate_async_eval_compile_on_off_have_same_output(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _enable_async_eval(monkeypatch)
    tokenizer = _FakeTokenizer({0: "<eos>", 1: "A", 2: "B", 3: "C", 9: "P"})
    transitions = _transition_row_fn({9: 1, 1: 2, 2: 3, 3: 0}, vocab_size=10)
...
def test_async_eval_quantized_kv_start_downgrades_compiled_runtime(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls = _enable_async_eval(monkeypatch)
    tokenizer = _FakeTokenizer({0: "<eos>", 1: "A", 2: "B", 3: "C", 9: "P"})
