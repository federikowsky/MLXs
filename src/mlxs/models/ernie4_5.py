"""ERNIE 4.5 model — port from mlx_lm (ernie4_5.py), ModelProtocol-compliant.

Dense decoder-only: RMSNorm, SwiGLU MLP, full RoPE (traditional), optional bias.
...
class Attention(nn.Module):
    ...
    def __init__(self, args: ModelArgs) -> None:
        ...
        self.q_proj = nn.Linear(dim, self.n_heads * self.head_dim, bias=args.use_bias)
        self.k_proj = nn.Linear(dim, self.n_kv_heads * self.head_dim, bias=args.use_bias)
        self.v_proj = nn.Linear(dim, self.n_kv_heads * self.head_dim, bias=args.use_bias)
        self.o_proj = nn.Linear(self.n_heads * self.head_dim, dim, bias=args.use_bias)
...
class Ernie45Model(nn.Module):
    """ERNIE 4.5 transformer: embed, decoder layers, final norm."""
...
class Model(nn.Module):
    """ERNIE 4.5 LM head wrapper — satisfies ModelProtocol."""
