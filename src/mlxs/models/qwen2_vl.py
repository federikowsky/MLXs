"""Qwen2-VL: dual-mode wrapper (text-only / multimodal) — ModelProtocol + MultimodalModelProtocol.
...
SigLIP vision encoder + projector and supports ``prepare_inputs`` for
image processing (FR12, §7.4, AC11).
"""
...
class ModelArgs(BaseModelArgs):
    """Qwen2-VL config: model_type + text_config + optional vision_config."""
...
class Model(nn.Module):
    """Qwen2-VL LM wrapper — dual-mode: text-only or multimodal (§7.4)."""
    ...
    def sanitize(self, weights: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._mode == ModelMode.TEXT:
            # Strip all vision weights (original behavior)
            filtered = {
        ...
        vision_weights = {}
        for k, v in filtered.items():
            # Remap "visual" -> "vision_tower" (HF convention)
