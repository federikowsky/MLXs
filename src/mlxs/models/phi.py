"""Phi model architecture (Phi-2).

Implements ModelProtocol. Compatible with mlx_lm-converted weights.
Supports partial rotary embeddings, GQA, and parallel attention+MLP blocks.
"""
...
class PhiAttention(nn.Module):
    """Multi-head attention with partial rotary embeddings."""
...
class PhiMLP(nn.Module):
    """Phi MLP with GELU approximate activation."""
...
class PhiDecoderLayer(nn.Module):
    """Parallel attention + MLP decoder layer."""
...
class Model(nn.Module):
    """Phi LM head wrapper — satisfies ModelProtocol."""

    def __init__(self, args: ModelArgs) -> None:
        super().__init__()
        self.args = args
        self.model_type = args.model_type
        self.model = PhiModel(args)
    ...
    @property
    def layers(self) -> list[PhiDecoderLayer]:
