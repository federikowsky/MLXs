"""Mamba state-space model — port from mlx_lm, ModelProtocol-compliant."""
...
class ModelArgs(BaseModelArgs):
    ...
    def __post_init__(self) -> None:
        ...
        if self.time_step_rank == "auto":
            ...
            self.time_step_rank = int(self.time_step_rank)
...
class MambaBlock(nn.Module):
    ...
    def ssm_step(
        ...
        norm_fn = self.mixer_norm if self.use_bcdt_rms else lambda x: x
        delta, B, C = map(
            norm_fn,
            mx.split(
                deltaBC,
    ...
    def __call__(self, x: mx.array, cache: ArraysCache | None) -> mx.array:
        if cache is None:
...
class Model(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        x = self.backbone(input_ids, cache)
    ...
    def make_cache(self) -> list[ArraysCache]:
        return [ArraysCache(size=2) for _ in range(len(self.backbone.layers))]
