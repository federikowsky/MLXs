"""Shared SSM (State Space Model) utilities.

Provides compute_dt, Metal kernel for fast SSM updates, and the
ssm_attn/ssm_update dispatcher. Used by Mamba, Mamba2, Jamba,
Falcon-H1, and other SSM-based architectures.

Compatible with mlx_lm ssm.py API.
"""
...
def make_ssm_kernel() -> mx.fast.metal_kernel | None:
...
def segsum(x: mx.array, mask: mx.array | None = None) -> mx.array:
    seq_len = x.shape[-1]
    if mask is not None:
...
def ssm_attn(
    ...
    dtx = dt.reshape(b, seq_len, h, 1) * x
    ...
    def _step(
        ...
        B_rep = mx.repeat(B, h // g, axis=1).swapaxes(2, 3)
        dtxdecay = dtx * decay
...
def ssm_update(
    ...
    """Dispatch to Metal kernel (single-step decode) or attention-based SSM."""
