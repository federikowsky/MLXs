...
def _initialize_rope(
    ...
    """Initialize RoPE with optional scaling (compatible with mlx_lm)."""
    if scaling is None:
    ...
    if rope_type == "llama3":
        ...
        old_context_len = scaling.get(
            "original_max_position_embeddings", max_position_embeddings or 8192
        )
        # Compute per-dimension scaling
        freqs = base ** (mx.arange(0, dims, 2, dtype=mx.float32) / dims)
        old_context_freqs = 2 * mx.pi / freqs
        wavelen = old_context_freqs
        ...
        return nn.RoPE(dims, traditional=traditional, base=base, freqs=new_freqs)
