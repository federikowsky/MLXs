"""Solar Open model — port from mlx_lm (solar_open.py), ModelProtocol-compliant.

Same architecture as GLM-4 MoE: dense + MoE layers, optional QK norm, RoPE (traditional=False),
group expert selection, shared experts. Solar-specific ModelArgs (n_shared_experts required, etc.).
...
class ModelArgs(BaseModelArgs):
    """Solar Open config (config.json). Matches mlx_lm solar_open.ModelArgs."""

    model_type: str = "solar_open"
...
class SolarOpenMoEBlock(nn.Module):
    ...
    def __init__(self, args: ModelArgs) -> None:
        ...
        self.gate = SolarOpenGate(args)
        self.shared_experts: SolarOpenMLP | None = None
        if args.n_shared_experts > 0:
            shared_intermediate = args.moe_intermediate_size * args.n_shared_experts
            self.shared_experts = SolarOpenMLP(args, intermediate_size=shared_intermediate)
...
class SolarOpenDecoderLayer(nn.Module):
    ...
    def __init__(self, args: ModelArgs, layer_idx: int) -> None:
        super().__init__()
        self.self_attn = SolarOpenAttention(args)
        use_moe = layer_idx >= args.first_k_dense_replace
        self.mlp = SolarOpenMoEBlock(args) if use_moe else SolarOpenMLP(args)
...
class SolarOpenModel(nn.Module):
    """Solar Open transformer: embed, decoder layers, final norm."""

    def __init__(self, args: ModelArgs) -> None:
        super().__init__()
        self.embed_tokens = nn.Embedding(args.vocab_size, args.hidden_size)
        self.layers = [SolarOpenDecoderLayer(args, idx) for idx in range(args.num_hidden_layers)]
...
class Model(nn.Module):
    """Solar Open LM head wrapper — satisfies ModelProtocol."""

    def __init__(self, args: ModelArgs) -> None:
        super().__init__()
        self.args = args
        self.model_type = args.model_type
        self.model = SolarOpenModel(args)
