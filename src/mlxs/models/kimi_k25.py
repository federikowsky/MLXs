"""Kimi K2.5: dual-mode wrapper (text-only / multimodal) — ModelProtocol + MultimodalModelProtocol.
...
class ModelArgs(BaseModelArgs):
    """Kimi K2.5 config: wraps DeepSeek V3 text_config + optional vision_config."""

    text_config: TextConfig | dict[str, Any] = None  # type: ignore[assignment]
    model_type: str = "kimi_k25"
    vision_config: dict[str, Any] | None = None
    image_token_id: int = 163606
...
class Model(nn.Module):
    """Kimi K2.5 LM wrapper — dual-mode: text-only or multimodal (§7.4)."""
