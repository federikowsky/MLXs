"""Family A runtime substrate: homogeneous token-addressable full-KV decoders."""
...
FULL_KV_FAMILY = RuntimeFamilyDescriptor(
    ...
    display_name="Homogeneous Full-KV Decoder",
    summary=(
        "Token-addressable per-layer KVCache state with exact replay/recovery and "
        "mixed-tier resident execution."
    ),
...
class FullAttentionKVRuntimeSubstrate(AdaptiveKVRuntimeSubstrate):
    """Runtime substrate for exact homogeneous full-KV decoder families."""
    ...
    def make_layer_runtime(
        ...
        return FullAttentionKVAdaptiveLayerCache(manager, layer_index=layer_index)
...
class FullAttentionKVAdaptiveLayerCache:
    """Per-layer adaptive cache wrapper for homogeneous full-KV runtimes."""
    ...
    def recover_blocks_from_scratch(
        ...
        if not blocks:
            return
        start_token = blocks[0].start_token
        end_token = blocks[-1].end_token
        ...
        self.recover_blocks(blocks, keys, values)
