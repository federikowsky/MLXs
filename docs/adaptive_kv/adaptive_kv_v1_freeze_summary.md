...
- **HARD stress scope:** Improved materially and now **closed for V1**. The later Exact Runtime Optimization Program and the final Runtime Architecture R&D program were executed seriously, validated, and are now **closed** with the existing retained baseline still in place; the deeper structural runtime/data-plane redesigns were **not** retained because they failed the real benchmark gate, including the decisive same-process 1024-token HARD gate. **Remaining gap vs full** is structural to the current exact mixed-tier segmented executor, not a sign of unfinished routine hardening. Further work here is **optional executor/data-plane R&D**, not normal V1 completion work.
...
- Continuing HARD-regime optimization, the Exact Runtime Optimization Program, or the later Runtime Architecture R&D program as ordinary V1 hardening; any further gain now belongs to explicit executor/data-plane R&D tied to product targets, with the current retained baseline otherwise left in place.
