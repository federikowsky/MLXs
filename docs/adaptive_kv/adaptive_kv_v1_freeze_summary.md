...
Single-request generation only, **`model_type == "llama"`**, full-attention Llama baseline, homogeneous `list[KVCache]` baseline, block-level policy with tiers **FULL / COMPRESSED / EVICTED**, real replay/recovery, observer-only semantics for usage, unsupported families rejected at compatibility check. The retained implementation baseline now uses an explicit generic adaptive-KV core plus adapter-backed runtime bindings and structured capability reporting, but the only retained working path remains the verified Llama adapter.
...
| **HARD** | 30_000_000 / 45_000_000 | After the dedicated HARD passes and a later exact runtime program, **`adaptive_hard` is considered good enough under stress for V1**. The retained runtime baseline keeps **executor-ready resident slice metadata**, a **score-side `q_len == 1` mixed-tier decode specialization**, and the **generic executor as exact fallback**. A final Runtime Architecture R&D program then explored deeper exact mixed-tier executor and resident-plan redesigns; those prototypes remained exact and were benchmarked seriously, but the streaming mixed-tier decode path regressed and the incremental resident-plan refresh path failed the decisive same-process 1024-token HARD gate, so both were reverted. A later Core Cleanup + Multi-Model Architecture Program then separated the generic adaptive-KV core from model/backend-specific runtime bindings, introduced an explicit capability model plus adapter-backed runtime selection, and migrated the retained Llama path behind the first concrete adapter. Controlled benchmark comparison found that cleanup effectively performance-neutral within noise, so this cleaned-up adapter-backed structure is now the retained baseline. Real support, however, remains the earlier verified Llama path only. |
...
- **HARD stress scope:** Improved materially and now **closed for V1**. The later Exact Runtime Optimization Program, the final Runtime Architecture R&D program, and the Core Cleanup + Multi-Model Architecture Program were executed seriously, validated, and are now **closed**. The deeper structural runtime/data-plane redesigns were **not** retained because they failed the real benchmark gate, while the later architecture cleanup was retained because it preserved behavior and remained effectively performance-neutral within controlled benchmark noise. The current retained baseline is therefore the cleaned-up adapter-backed architecture with the same verified Llama runtime path. **Remaining gap vs full** is structural to the current exact mixed-tier segmented executor, not a sign of unfinished routine hardening. Further work here is **optional executor/data-plane R&D**; future model-family expansion should build on the retained capability/adapter structure, not bypass it.
...
- Continuing HARD-regime optimization, the Exact Runtime Optimization Program, the later Runtime Architecture R&D program, or the Core Cleanup + Multi-Model Architecture Program as ordinary V1 hardening; any further gain now belongs to explicit executor/data-plane R&D tied to product targets, and any broader support work should extend the retained capability/adapter baseline rather than implying support automatically.
