...
| **HARD** | 30_000_000 / 45_000_000 | After the dedicated HARD passes and a later exact runtime program, **`adaptive_hard` is considered good enough under stress for V1**. The retained runtime subset kept **executor-ready resident slice metadata**, a **score-side `q_len == 1` mixed-tier decode specialization**, and the **generic executor as exact fallback**, while a broader executor-fusion attempt was benchmark-negative and reverted. On fresh same-machine reruns of `hard_pressure_context`, **`adaptive_hard` improved from 185.29 to 210.49 tok/s at 512 prompt tokens (+13.6%)** and from **212.61 to 220.47 tok/s at 1024 prompt tokens (+3.7%)**, with correctness intact and recompute/eviction/pressure/resident-byte signals semantically consistent. The remaining gap is still attributed to the current exact mixed-tier segmented executor, not an obvious local overhead bug. |
...
- Continuing HARD-regime optimization or the Exact Runtime Optimization Program as ordinary V1 hardening; any further gain now belongs to explicit executor R&D tied to product targets.
