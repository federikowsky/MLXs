...
Single-request generation only, **`model_type == "llama"`**, full-attention Llama baseline, homogeneous `list[KVCache]` baseline, block-level policy with tiers **FULL / COMPRESSED / EVICTED**, real replay/recovery, observer-only semantics for usage, unsupported families rejected at compatibility check. The retained implementation baseline now uses an explicit platform architecture: a generic adaptive-KV core separated from capability provider, runtime substrate, replay backend, and composed runtime adapter roles. The only retained working path, however, remains the verified Llama implementation migrated onto that structure.
...
1. **`decode_loop`** (`src/mlxs/generate/decode.py`): generic orchestration in the manager drives `before_decode_forward` → `ensure_required_resident` (replay if needed) → model forward → `after_decode_forward`, while runtime-specific resident/replay behavior is supplied by abstract runtime substrate and replay backend components selected through the current platform adapter. In the retained V1 path, that composed adapter is still Llama-only.
...
| **HARD** | 30_000_000 / 45_000_000 | After the dedicated HARD passes and a later exact runtime program, **`adaptive_hard` is considered good enough under stress for V1**. The retained runtime baseline keeps **executor-ready resident slice metadata**, a **score-side `q_len == 1` mixed-tier decode specialization**, and the **generic executor as exact fallback**. A final Runtime Architecture R&D program then explored deeper exact mixed-tier executor and resident-plan redesigns; those prototypes remained exact and were benchmarked seriously, but the streaming mixed-tier decode path regressed and the incremental resident-plan refresh path failed the decisive same-process 1024-token HARD gate, so both were reverted. Later architecture work then separated the generic adaptive-KV core from model/backend-specific runtime bindings, introduced explicit capability-provider / runtime-substrate / replay-backend roles under a composed runtime adapter, and fully migrated the retained Llama path onto that platform structure. Fresh same-machine regression checks preserved correctness and support honesty and did not show a material retained-path regression, so this platform architecture is now the retained baseline. Real support, however, remains the earlier verified Llama path only. |
...
- **HARD stress scope:** Improved materially and now **closed for V1**. The later Exact Runtime Optimization Program, the final Runtime Architecture R&D program, the Core Cleanup + Multi-Model Architecture Program, and the final Platform Architecture Refactor were executed seriously, validated, and are now **closed**. The deeper structural runtime/data-plane redesigns were **not** retained because they failed the real benchmark gate, while the later platform refactor was retained because it preserved behavior, support honesty, and remained effectively performance-neutral within the fresh regression checks. The current retained baseline is therefore the platform architecture with the same verified Llama runtime path. **Remaining gap vs full** is structural to the current exact mixed-tier segmented executor, not a sign of unfinished routine hardening. Further work here is **optional executor/data-plane R&D**; future model-family expansion should build on the retained platform contracts, not bypass them.
...
- Continuing HARD-regime optimization, the Exact Runtime Optimization Program, the later Runtime Architecture R&D program, the Core Cleanup + Multi-Model Architecture Program, or the Platform Architecture Refactor as ordinary V1 hardening; any further gain now belongs to explicit executor/data-plane R&D tied to product targets, and any broader support work should extend the retained platform contracts rather than implying support automatically.
