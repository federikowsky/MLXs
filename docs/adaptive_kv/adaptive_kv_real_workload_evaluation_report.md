...
**Regression anchor:** Re-run **§3** when changing adaptive attention, budgets, or workload excerpts; compare **`reference_token_match`** and throughput to this JSON. Token expectations: **`adaptive_kv_v1_freeze_summary.md`** (§7). Optional **compressed-KV fidelity** work remains product-driven. Later post-freeze runtime work kept **executor-ready resident slice metadata** and a **score-side `q_len == 1` mixed-tier specialization**, reverted a broader benchmark-negative executor fusion attempt, and left the remaining 1024-token HARD gap in optional deeper executor R&D territory rather than routine hardening.
