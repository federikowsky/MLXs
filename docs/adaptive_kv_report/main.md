# Adaptive KV for MLXs: Exact Adaptive KV-Cache Management for Single-Request Llama Inference
...
That design choice makes the systems problem harder, not easier. It introduces a control plane, a resident-state representation, recovery logic, and mixed-tier attention execution. The work reported here is therefore not just a policy sketch. It is a retained implementation in MLXs, with repeated synthetic benchmarks, focused parity investigations, real-workload evaluation, and a later architecture cleanup that turned the system into a cleaner platform baseline for future extension.
...
| Model family | Verified Llama path only |
| Baseline attention family | Standard full-attention Llama baseline |
...
| Unsupported in V1 | `compile_decode`, legacy `quantized_kv_start`, external cache reuse, unsupported families |
...
### 5.2 Platform Runtime Structure

The retained runtime boundary is now explicit. The platform architecture separates four roles:
...
4. **Composed runtime adapter**  
   Binds the three components above into a concrete family/runtime implementation.

The manager now orchestrates abstract runtime and replay components rather than depending on a Llama-specific architectural assumption.

### 5.3 Retained Llama Path

The current verified path remains the full-attention Llama baseline. In the retained architecture:

- support assessment is handled by the Llama capability provider,
- per-layer runtime behavior is supplied by the Llama runtime substrate,
- replay-backed recovery is supplied by the Llama replay backend,
- and those pieces are composed into the retained Llama adapter.

No broader working support is implied by this structure. The platform architecture is a cleaner basis for future work, not evidence that additional families already function.
...
|---|---|
| `adaptive_full` | Expected to match the non-adaptive greedy reference under the supported Llama baseline |
...
The retained platform architecture exposes support explicitly through a capability model with `FULL`, `PARTIAL`, and `UNSUPPORTED` outcomes. Unsupported families and unsupported configurations fail clearly instead of appearing to work accidentally.
...
### 8.5 Architecture-Level Regression Checks

Later platform-architecture refactors were accepted only if they preserved the retained path within benchmark noise. Same-machine regression checks did not show a material retained-path regression. In particular, the within-run adaptive/full ratios were stable or slightly improved:

| Case | Before ratio | After ratio |
|---|---:|---:|
...
These checks are important because they justify retaining the cleaned platform architecture as the implementation baseline rather than treating it as a purely cosmetic refactor.
...
| Platform architecture refactor | The retained implementation was correct but too family-specific internally | Separated capability provider, runtime substrate, replay backend, and composed runtime adapter | Produced the current retained baseline and the intended foundation for future family support |
...
- verified Llama full-attention path only,
- no claim of broader working family support,
...
3. **Explicit multi-family expansion**  
   The retained platform architecture is now designed so future model families can plug in through capability, runtime, and replay components. That architecture is the foundation for future expansion, but it does not itself imply that additional families are already supported.

## 13. Conclusion

Adaptive KV for MLXs is now a retained, bounded exact system rather than a speculative design. It manages KV residency at block granularity, preserves attention semantics over the resident state, uses real replay-backed recovery for evicted content, and exposes support honestly through an explicit platform architecture.

Within its approved scope, the result is clear. `adaptive_full` is aligned with the supported non-adaptive baseline, `adaptive_soft` is performant enough in the normal compression regime to be practically useful, and `adaptive_hard` is semantically safe and good enough under stress even though its remaining throughput gap is structural. The later architecture refactors strengthened the implementation boundary without materially regressing the retained path, so the platform architecture is now the correct baseline for future work.

That is the right final position for V1: not universal support, not maximal throughput under every pressure regime, but a serious exact implementation with clear guarantees, honest limits, and a clean architectural foundation for any future expansion.
